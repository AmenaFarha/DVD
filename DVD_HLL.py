...
class KMeansRunner:
    ...
    def execute(self):
        ...
        for item in clustered_data.tolist():
          hll.add(data_to_bytes(item))  # you can add raw strings or bytes
        ...
        blk_data = {
            ...
            'hll_estimate': len(hll),   # pip library: cardinality = len(hll)
            'error_rate': 0.01                  # store precision so validators know expected error
        }
